...
export function getGovtToken(): string | null {
  return localStorage.getItem(GOVT_TOKEN_KEY);
}

export function setGovtToken(token: string): void {
  localStorage.setItem(GOVT_TOKEN_KEY, token);
}

export function removeGovtToken(): void {
...
export function getGovtUser(): GovtUser | null {
  const token = getGovtToken();
...
export function govtLogout(): void {
  removeGovtToken();
}
...
export async function govtLoginApi(email: string, password: string) {
  ...
  if (data?.token) {
    setGovtToken(data.token);
  }
  return data;
}

export async function govtSignupApi(
  email: string,
  password: string,
  full_name: string,
  department: string
